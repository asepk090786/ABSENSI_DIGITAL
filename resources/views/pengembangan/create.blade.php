...
@section('content')
    ...
    <form method="POST" action="{{ route('pengembangan.store') }}">
        ...
        <div class="mb-3">
            ...
            <select name="kegiatan_id" id="kegiatanSelect" class="form-control">
                ...
                @foreach($kegiatanList as $kg)
                    <option value="{{ $kg->id }}" data-kategori="{{ $kg->kategori }}" {{ old('kegiatan_id') == $kg->id ? 'selected' : '' }}>{{ $kg->nama_kegiatan }} ({{ $kg->kode_kegiatan ?? '' }})</option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            <select name="jenis_kegiatan" class="form-control">
                ...
                @foreach($jenisList as $jk)
                    <option value="{{ $jk->kode }}" {{ old('jenis_kegiatan') == $jk->kode ? 'selected' : '' }}>{{ $jk->nama }} ({{ $jk->kode }})</option>
                @endforeach
            </select>
        </div>
        <div class="mb-3">
        ...
        <div class="mb-3">
            ...
            <input name="pemateri_names" class="form-control" value="{{ old('pemateri_names') }}" placeholder="Nama Pemateri Tambahan, Contoh: Narasumber A, Narasumber B" />
        </div>
        <div class="mb-3">
        ...
        @php
            $selectedExternal = old('external_participants', []);
        @endphp
        <div class="mb-3">
                            ...
                            @foreach($gurus as $g)
                                <div class="col-md-6">
                                    <div class="form-check">
                                        <input class="form-check-input" type="checkbox" name="pemateri_guru_ids[]" value="{{ $g->id }}" id="pemateriGuru{{ $g->id }}" {{ in_array($g->id, old('pemateri_guru_ids', [])) ? 'checked' : '' }}>
                    ...
                    <div class="modal-body">
                        ...
                        <div class="row g-2">
                            @foreach($gurus as $g)
                                <div class="col-12">
                                    <div class="form-check">
                                        <input class="form-check-input" type="checkbox" name="guru_ids[]" value="{{ $g->id }}" id="pesertaGuru{{ $g->id }}" {{ in_array($g->id, old('guru_ids', [])) ? 'checked' : '' }}>
                    ...
                    <div class="modal-body">
                        ...
                        <div class="row g-2">
                            @foreach($siswas as $s)
                                <div class="col-12">
                                    <div class="form-check">
                                        <input class="form-check-input" type="checkbox" name="siswa_ids[]" value="{{ $s->id }}" id="pesertaSiswa{{ $s->id }}" {{ in_array($s->id, old('siswa_ids', [])) ? 'checked' : '' }}>
...
document.addEventListener('DOMContentLoaded', function(){
    ...
    function renderTable(tableId, headers, rows) {
        ...
        const tbody = rows.length === 0
            ? `<tbody><tr><td colspan="${headers.length}" class="text-muted">Belum ada data dipilih</td></tr></tbody>`
            : `<tbody>${rows.map(row => `<tr>${row.map(cell => `<td>${cell}</td>`).join('')}</tr>`).join('')}</tbody>`;
        table.innerHTML = thead + tbody;
    }

    function getCheckedLabel(cb) {
        const label = document.querySelector(`label[for="${cb.id}"]`);
        return label ? label.textContent.trim() : cb.value;
    }

    function renderSelectedPemateri() {
        const checked = [...document.querySelectorAll('#pemateriModal input[name="pemateri_guru_ids[]"]:checked')];
        const rows = checked.map(cb => [getCheckedLabel(cb)]);
        renderTable('selectedPemateriTable', ['Nama Pemateri'], rows);
    }

    function renderSelectedPeserta() {
        const rows = [];
        [...document.querySelectorAll('#guruPesertaModal input[name="guru_ids[]"]:checked')].forEach(cb => rows.push(['Guru', getCheckedLabel(cb)]));
        [...document.querySelectorAll('#siswaPesertaModal input[name="siswa_ids[]"]:checked')].forEach(cb => rows.push(['Siswa', getCheckedLabel(cb)]));
        renderTable('selectedPesertaTable', ['Tipe', 'Nama Peserta'], rows);
    }
