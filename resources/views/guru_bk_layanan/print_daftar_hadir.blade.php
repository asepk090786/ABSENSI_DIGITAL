...
<head>
    ...
    <title>Daftar Hadir Layanan BK - {{ $sekolah->nama_sekolah ?? 'Sekolah' }}</title>
    <style>
...
<body>
    ...
    <div class="header">
        <p class="header-title">Daftar Hadir Layanan BK</p>
    ...
    <div class="info">
        ...
        @if(!empty($selectedTanggal))
            | <strong>Tanggal:</strong> {{ \Carbon\Carbon::parse($selectedTanggal)->format('d/m/Y') }}
        @endif
    </div>
    ...
    <table>
        ...
        <tbody>
            @forelse($daftarHadirItems as $index => $item)
                <tr>
