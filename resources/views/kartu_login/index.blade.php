...
<div class="container-fluid kartu-login-page">
    ...
    <div id="emptyPreview" class="card d-print-none">
...
@push('css')
<style>
    .login-cards-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(322px, 1fr)); gap: 1.5rem; align-items: start; }
    .login-card { display: none; overflow: hidden; width: 322px; height: 460px; min-height: 460px; background: #fff; border: 1px solid #e2e8f0; border-radius: 24px; box-shadow: 0 18px 45px rgba(15, 23, 42, .11), 0 3px 10px rgba(37, 99, 235, .05); }
    .login-card-accent { height: 2.5mm; background: linear-gradient(100deg, #2563eb 0%, #3b82f6 48%, #10b981 100%); }
    .login-card-body { display: flex; flex-direction: column; height: calc(100% - 2.5mm); padding: 3mm; }
    .login-card-header { display: flex; align-items: flex-start; justify-content: space-between; margin-bottom: 2mm; }
    ...
    .login-card-profile { display: flex; flex-direction: column; align-items: center; gap: 1.5mm; min-width: 0; margin: 0 0 2mm; padding: 2mm; background: #f8fafc; border: 1px solid #eef2f7; border-radius: 12px; text-align: center; }
    ...
    .login-card-qr { display: flex; align-items: center; gap: 3mm; min-width: 0; margin: 0 0 1mm; padding: 2mm; background: linear-gradient(135deg, #f8fbff 0%, #eff6ff 100%); border: 1px solid #dbeafe; border-radius: 12px; }
    .login-card-qr-image { flex: 0 0 24mm; width: 24mm; height: 24mm; padding: 1mm; background: #fff; border: 1px solid #e2e8f0; border-radius: 8px; box-shadow: 0 5px 15px rgba(37, 99, 235, .08); }
    ...
    .login-card-footer { display: flex; align-items: center; gap: 2mm; margin-top: auto; padding: 1mm 2mm; color: #1e40af; background: #eff6ff; border-radius: 10px; font-size: .54rem; }
    .login-card-footer strong, .login-card-footer span { display: block; }
    .login-card-footer strong { font-size: .56rem; font-weight: 800; }
    .login-card-footer span { margin-top: .12rem; color: #64748b; }
    .login-card-security-icon { display: flex; align-items: center; justify-content: center; flex: 0 0 7mm; width: 7mm; height: 7mm; color: #fff; background: #2563eb; border-radius: 7px; font-size: .9rem; }
    @media print {
        ...
        #loginCardsPreview { position: absolute; top: 0; left: 0; width: 100%; }
        .login-cards-grid { grid-template-columns: repeat(2, 70mm); gap: 8mm; }
        .login-card { width: 70mm; height: 100mm; min-height: 100mm; break-inside: avoid; box-shadow: none; }
        .login-card-body { height: calc(100mm - 2.5mm); padding: 3mm; }
    }
    @media (max-width: 575px) {
        .login-cards-grid { grid-template-columns: 1fr; gap: 1rem; }
        .login-card { width: 100%; height: auto; min-height: 460px; }
