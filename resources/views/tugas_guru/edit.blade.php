        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header border-0 pt-3 pb-2">
                    <h3 class="card-title">Edit Tugas Guru</h3>
                </div>
                <div class="card-body">
                    @if(session('error'))
                    ...
                    <form action="{{ route('tugas_guru.update', $tugas_guru->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                                ...
                                <div class="mb-2">
                                    ...
                                    <select name="guru_id" id="guru_id" class="form-select @error('guru_id') is-invalid @enderror" required>
                                        ...
                                        @foreach($guruList as $guru)
                                            <option value="{{ $guru->id }}" {{ old('guru_id', $tugas_guru->guru_id) == $guru->id ? 'selected' : '' }}>
                                ...
                                <div class="mb-2">
                                    ...
                                    <select name="mata_pelajaran_id" id="mata_pelajaran_id" class="form-select @error('mata_pelajaran_id') is-invalid @enderror" required>
                                        ...
                                        @foreach($mataPelajaranList as $mapel)
                                            <option value="{{ $mapel->id }}" {{ old('mata_pelajaran_id', $tugas_guru->mata_pelajaran_id) == $mapel->id ? 'selected' : '' }}>
                                ...
                                <div class="mb-2">
                                    ...
                                    <select name="tingkat_kelas" id="tingkat_kelas" class="form-select @error('tingkat_kelas') is-invalid @enderror" required>
                                        ...
                                        @foreach($tingkatList as $tingkat)
                                            <option value="{{ $tingkat }}" {{ old('tingkat_kelas', $tugas_guru->tingkat_kelas) == $tingkat ? 'selected' : '' }}>
                                                {{ $tingkat }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="mb-2">
                                    ...
                                    <select name="kelas_id" id="kelas_id" class="form-select @error('kelas_id') is-invalid @enderror">
                                        ...
                                        @foreach($kelasList as $kelas)
                                            <option value="{{ $kelas->id }}" data-tingkat="{{ $kelas->tingkat_kelas }}" {{ old('kelas_id', $tugas_guru->kelas_id) == $kelas->id ? 'selected' : '' }}>
                                                {{ $kelas->nama_kelas }}
                                            </option>
                                        @endforeach
                                    </select>
                        ...
                        <div class="mb-2">
                            <div class="form-check form-switch">
                                <input class="form-check-input" type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $tugas_guru->is_active) ? 'checked' : '' }}>
                        ...
                        <div class="mb-2">
                            ...
                            <textarea name="keterangan" id="keterangan" class="form-control @error('keterangan') is-invalid @enderror" rows="3">{{ old('keterangan', $tugas_guru->keterangan) }}</textarea>
                            @error('keterangan')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
